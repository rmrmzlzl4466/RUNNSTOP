...
export function spawnItemAtCol(runtime, rowIdx, type, col) {
...
export function pruneItemsBehindPlayer(runtime, player) {
  for (let i = runtime.items.length - 1; i >= 0; i--) {
    if (runtime.items[i].y >= player.y) runtime.items.splice(i, 1);
  }
}

export function applyItem(runtime, player, qaConfig, item) {
  if (item.type === 'bit') {
